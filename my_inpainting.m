...
%   
%
function [inpaintedImg,C,D,fillMovie] = my_inpainting2(origImg,mask,psz,npsz,N,errortolerance,belta)
...
%         struct_sparse(k) = norm(WW(k,:))*sqrt(countp/numel(Hnp));
  ...
  %vvlambda = zeros(sz);
  %vvlambda(dR2)=Lambda(dR2)/(Mlambda+0.001);
  %priorities = C(dR2).* (0.5*D(dR2).*vvlambda(dR2)+0.5*D(dR2));
  ...
  if rows>0
      ...
      %end
      [~,approxpatch]=l0_inpainting(psz,N,WW,Hpj,dd,ReorigImg,Hp,ndx(1),toFill,Hq,errortolerance,belta);
